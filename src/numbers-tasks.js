...
function getAverage(value1, value2) {
  const summa = value1 + value2;
  if (summa > Number.MAX_VALUE) {
...
function getLinearEquationRoot(a, b) {
  if (a === 0) {
    throw new Error('a cannot be equal to zero');
  }
  return -b / a;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const summa = x1 * x2 + y1 * y2;
  const magnitude1 = Math.sqrt(x1 * x1 + y1 * y1);
  const magnitude2 = Math.sqrt(x2 * x2 + y2 * y2);
  if (magnitude1 === 0 || magnitude2 === 0) {
    throw new Error('The length of each vectors must not be equel to zero');
  }
  const cos = summa / (magnitude1 * magnitude2);
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    const nextIndex = oldIndex + newIndex;
    oldIndex = newIndex;
    newIndex = nextIndex;
  }
  return newIndex;
